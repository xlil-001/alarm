...
class AlarmApp(App):
    ...
    def show_add_dialog(self):
        ...
        # 行1：小时
        row_hour = BoxLayout(size_hint_y=None, height='42dp')
        row_hour.add_widget(Label(text='小时 (0-23)', size_hint_x=0.35, font_name=FONT_PATH))
        hour_input = TextInput(text='7', input_filter='int', multiline=False,
                               font_name=FONT_PATH, size_hint_x=0.65)
        row_hour.add_widget(hour_input)
        scroll_content.add_widget(row_hour)

        # 行2：分钟
        row_min = BoxLayout(size_hint_y=None, height='42dp')
        row_min.add_widget(Label(text='分钟 (0-59)', size_hint_x=0.35, font_name=FONT_PATH))
        minute_input = TextInput(text='30', input_filter='int', multiline=False,
                                 font_name=FONT_PATH, size_hint_x=0.65)
        row_min.add_widget(minute_input)
        scroll_content.add_widget(row_min)

        # 行3：重复日（标签置顶，按钮紧随，整体高度增加）
        row_days = BoxLayout(orientation='vertical', size_hint_y=None, height='75dp', spacing=2)
        lbl_day = Label(text='重复日', size_hint_y=None, height='20dp',
                        font_name=FONT_PATH, halign='left', valign='middle')
        ...
        # 行4：振动开关
        row_vib = BoxLayout(size_hint_y=None, height='42dp')
        row_vib.add_widget(Label(text='振动', size_hint_x=0.35, font_name=FONT_PATH))
        vibrate_switch = Switch(active=True, size_hint_x=0.65)
        row_vib.add_widget(vibrate_switch)
        scroll_content.add_widget(row_vib)

        # 行5：振动强度
        row_str = BoxLayout(size_hint_y=None, height='42dp')
        row_str.add_widget(Label(text='强度 (1~5)', size_hint_x=0.35, font_name=FONT_PATH))
        strength_slider = Slider(min=1, max=5, value=3, step=1, size_hint_x=0.65)
        row_str.add_widget(strength_slider)
        scroll_content.add_widget(row_str)
        ...
        # 行7：贪睡时间
        row_snooze = BoxLayout(size_hint_y=None, height='42dp')
        row_snooze.add_widget(Label(text='贪睡时间 (分)', size_hint_x=0.35, font_name=FONT_PATH))
        snooze_slider = Slider(min=1, max=30, value=5, step=1, size_hint_x=0.65)
        row_snooze.add_widget(snooze_slider)
        scroll_content.add_widget(row_snooze)

        scroll_view = ScrollView(size_hint_y=0.85)  # 留给保存按钮 0.15 的空间
